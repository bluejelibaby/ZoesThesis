...
Dropping contributions from possible signal to the likelihood (setting $s^{i} = s^{i}_{\mu} = 0$) allows consideration of the background-only hypothesis. The likelihood is maximised over all of the parameters with the tools Minuit~\cite{minuit} and RooFit for the data observations, the value of which is recorded ($L^{data}_{max}$). The values of the parameters at this maximum value are maximum likelihood estimates (MLEs). 
...
The results of the fit are shown in Figure~\ref{fig:fit}

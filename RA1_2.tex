...
Setting $s^{i} = s^{i}_{\mu} = 0$ allows the use of the likelihood to consider the background-only hypothesis. Finding the maximum of the likelihood for the observation from data ($L^{data}_{max}$) allows values of the parameters at maximum to be found, and these values are used to generate a probability distribution function. From this it is possible to generate a set of pseudo-experiments, and maximise the likelihood for each. This yields a distribution of the value of the maximum L_{max}, from which the $p$-value of the $L^{data}_{max}$ can be found using its quantile. 

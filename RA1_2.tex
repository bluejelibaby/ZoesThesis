...
Datasets of simulated events with calculated cross-sections are required for any analysis at the LHC. Using PDFs from CTEQ~\cite{CTEQ}, partons are generated and hard-scattered using either MadGraph~\cite{madgraph} or \textsc{pythia}~\cite{pythia}. They are hadronised using \textsc{pythia}, the exact parameters of which are extracted using a tune, in this case ``Z2"~\cite{tuneZ2}.

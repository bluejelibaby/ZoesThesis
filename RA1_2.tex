...
The number of events measured in data W$^{\mu}_{data, meas}$ is related to the actual total number of W $\ra \mu \nu$ events W$^{\mu}_{actual}$ by the relation in Equation~\ref{eq:Wa} where the purity $p$ represents the fraction of measured events 

\begin{equation}
W^{\mu}_{data, actual} = W^{\mu}_{data, meas} \times 
\label{eq:Wa}
\end{equation}
...
\begin{itemize}
\item The largest contribution to the overall uncertainty is from the uncertainty of the probability $p$ for a W decay to pass the lepton veto, divided into individual uncertainties on each type of decay averaged whils weighted by the frequency of that decay as described earlier in Section~\ref{sec:ttwcomp}. Overall this contributes 23\%.
...
\subsection{\tto + W Background Prediction Calculation}
